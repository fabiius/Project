(() => {
  ...
  const fields=t=>[t.total_base,t.contacted,t.confirmed,t.not_confirmed,t.does_not_know,t.mailbox,pct(t.coverage),pct(t.confirmation),t.pending];
  const headings=['Base','Contatados','Confirmados','Não confirmados','Não conhece','Caixa postal','Cobertura','Confirmação','Pendentes'];
  function makeSheets(r) {
    const common=[['Gerado em',r.timestamp],['Escopo',r.scope],['Filtros',r.scopeDetail],['Período',r.timeRule],['Rankings',r.rankRule],['Inconsistências',r.warning||'Nenhuma detectada.'],['Gráficos','Imagens incorporadas: não editáveis. Dados completos nas outras abas.'],['Cobertura','Contatados / base.'],['Confirmação','Confirmados / contatados.'],['Contatados','Confirmados + não confirmados + não conhece + caixa postal.'],['Pendentes','Base - contatados.'],['Armazenamento','Download local; nenhum relatório gravado no banco.']];
    for(let i=0;i<r.notes.length;i+=250)common.push([i?'Observações (continuação)':'Observações',r.notes.slice(i,i+250)]);
    return [
      {name:'Resumo executivo',headers:['Indicador','Valor'],widths:[28,28,22,22,22,22],rows:[['Base total',r.total.total_base],['Contatados',r.total.contacted],['Cobertura',pct(r.total.coverage)],['Confirmados',r.total.confirmed],['Taxa de confirmação',pct(r.total.confirmation)],['Pendentes',r.total.pending],['Registros',r.data.length],['Registros inconsistentes',r.invalid.length]],images:true},
      ...
      {name:'Rankings',headers:['Grupo','Critério','Posição','Coordenador','Líder','Base','Contatados','Confirmados','Cobertura','Confirmação'],widths:[20,26,12,30,30,18,18,18,18,18],rows:r.rankings.flatMap(rank=>rank.rows.map((t,i)=>[rank.kind,rank.title,i+1,t.coordinator||'Sem coordenador',t.leader||'',t.total_base,t.contacted,t.confirmed,pct(t.coverage),pct(t.confirmation)]))},
      {name:'Matriz completa',headers:['ID','Coordenador','Líder',...headings,'Atualização','Validação'],widths:[12,30,30,...Array(9).fill(18),28,60],rows:r.data.map(row=>[String(row.id??''),row.coordinator||'',row.leader||'',...fields(M.summarize([row])),row.updated_at||row.created_at||'',M.validate(row)||'OK']),total:['','Consolidado','',...fields(r.total),'','']},
  ...
  async function build(report,charts) {
    ...
    const zip=new window.JSZip(), sheets=makeSheets(report);
    ...
    zip.file('xl/_rels/workbook.xml.rels',`<Relationships xmlns="${relns}">${sheets.map((s,i)=>`<Relationship Id="rId${i+1}" Type="${office}/worksheet" Target="worksheets/sheet${i+1}.xml"/>`).join('')}<Relationship Id="rId7" Type="${office}/styles" Target="styles.xml"/></Relationships>`);
    ...
    sheets.forEach((sheet,index)=>{
      ...
      if(sheet.images)for(let n=15;n<=96;n++)rows.push(row(n===16?['Gráficos incorporados como imagens; não editáveis.']:[],n,20));
      const lastRow=sheet.images?96:sheet.rows.length+7;
    ...
    const chartNames=['status','coverage','confirmation'];
    ...
    zip.file('xl/drawings/drawing1.xml',`<xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="${office}">${chartNames.map((name,i)=>`<xdr:oneCellAnchor><xdr:from><xdr:col>0</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>${18+i*25}</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:from><xdr:ext cx="8191500" cy="3978911"/><xdr:pic><xdr:nvPicPr><xdr:cNvPr id="${i+1}" name="${name}"/><xdr:cNvPicPr><a:picLocks noChangeAspect="1"/></xdr:cNvPicPr></xdr:nvPicPr><xdr:blipFill><a:blip r:embed="rId${i+1}"/><a:stretch><a:fillRect/></a:stretch></xdr:blipFill><xdr:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="8191500" cy="3978911"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr></xdr:pic><xdr:clientData/></xdr:oneCellAnchor>`).join('')}</xdr:wsDr>`);
