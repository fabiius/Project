(() => {
  const cfg = window.APP_CONFIG || {}; if (!cfg.SUPABASE_URL || cfg.SUPABASE_URL.includes('COLE_AQUI')) { location.replace('login.html'); return; }
  ...
  async function start() {
    ...
    const {data:p,error} = await db.from('profiles').select('display_name,role').single(); if (error) { alert('Não foi possível carregar seu perfil: '+error.message); return; }
    admin = p.role === 'admin'; document.querySelector('#user-name').textContent = p.display_name; document.querySelector('#role-badge').textContent = admin ? 'Administrador' : 'Usuário';
    document.querySelector('#logout').onclick = async () => { await db.auth.signOut(); location.replace('login.html'); };
    document.querySelector('#add').onclick = add; document.querySelector('#filter').onchange = e => { selected=e.target.value; render(); }; await load();
  }
  ...
  function rows(data) {
    document.querySelector('#rows').innerHTML=data.map(r=>`<tr data-id="${r.id}">${fields.map(f=>`<td><input class="cell" data-field="${f}" ${numeric.has(f)?'type="number" min="0"':''} value="${esc(r[f])}"></td>`).join('')}<td class="font-bold text-indigo-600">${contacted(r)}</td><td class="font-bold">${rate(r).toFixed(1).replace('.',',')}%</td><td>${admin?'<button data-delete class="text-rose-600 font-semibold">Excluir</button>':'—'}</td></tr>`).join('');
    document.querySelectorAll('#rows input').forEach(i=>i.onchange=save); document.querySelectorAll('[data-delete]').forEach(b=>b.onclick=()=>remove(+b.closest('tr').dataset.id));
  ...
  window.salvarExcel=()=>{const b=XLSX.utils.book_new();XLSX.utils.book_append_sheet(b,XLSX.utils.json_to_sheet(exportRows(shown())),'Matriz Qualidade');XLSX.writeFile(b,`qualidade_matriz_${stamp()}.xlsx`);};
  window.baixarResumoDoDiaExcel=()=>{const b=XLSX.utils.book_new(),base=records.reduce((n,r)=>n+(+r.total_base||0),0),c=records.reduce((n,r)=>n+contacted(r),0),ok=records.reduce((n,r)=>n+(+r.confirmed||0),0);XLSX.utils.book_append_sheet(b,XLSX.utils.json_to_sheet([{'Métrica':'Data do relatório','Valor':new Date().toLocaleDateString('pt-BR')},{'Métrica':'Total Base','Valor':base},{'Métrica':'Pessoas contatadas','Valor':c},{'Métrica':'Taxa de confirmação','Valor':`${c?(ok*100/c).toFixed(1):'0.0'}%`}]),'Resumo Executivo');XLSX.utils.book_append_sheet(b,XLSX.utils.json_to_sheet(exportRows(records)),'Detalhamento');XLSX.writeFile(b,`resumo_diario_qualidade_${stamp()}.xlsx`);};
  window.exportarBackupJSON=()=>download(JSON.stringify(records,null,2),`backup_amostragem_${stamp()}.json`,'application/json;charset=utf-8');
  window.exportarXML=()=>{const x=['<?xml version="1.0" encoding="UTF-8"?>','<relatorio_qualidade>',...shown().map(r=>`<item><id>${r.id}</id><coordenador>${esc(r.coordinator)}</coordenador><lider>${esc(r.leader)}</lider><total>${r.total_base}</total><confirmado>${r.confirmed}</confirmado></item>`),'</relatorio_qualidade>'].join('\n');download(x,`relatorio_qualidade_${stamp()}.xml`,'application/xml;charset=utf-8');};
  start();
})();
